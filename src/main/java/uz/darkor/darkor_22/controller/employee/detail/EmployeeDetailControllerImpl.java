...
public class EmployeeDetailControllerImpl extends AbstractController<EmployeeDetailService> implements EmployeeDetailController {
    ...
    @GetMapping("get_by_employee/{code}")
    ...
    @GetMapping("get_to_update/{code}")
    public ResponseEntity<Data<EmployeeDetailUpdateDTO>> getToUpdate(@PathVariable UUID code, @RequestHeader("accept-language") String lang) {
        BaseUtils.setSessionLang(lang);
        return new ResponseEntity<>(new Data<>(service.getToUpdate(code)), HttpStatus.OK);
    }

    @GetMapping("get_by_course/{code}")
    public ResponseEntity<Data<List<EmployeeDetailLocalizedDTO>>> getByCourseCode(@PathVariable UUID code, @RequestHeader("accept-language") String lang) {
        BaseUtils.setSessionLang(lang);
        return new ResponseEntity<>(new Data<>(service.getAllByCourse(code)), HttpStatus.OK);
    }
}