...
public class StatisticsServiceImpl extends AbstractService<StatisticsMapper, StatisticsRepository> {
    ...
    public StatisticsLocalizedDTO get(Long id,StatisticsType type, String language) {

        Statistics statistics = repository.findByIdAndStatisticsType(id,type)
                .orElseThrow(() -> new NotFoundException("Not Found"));
    ...
    public List<StatisticsLocalizedDTO> list(StatisticsCriteria criteria,StatisticsType type, String language) {
        List<StatisticsLocalizedDTO> statisticsGetDTOS = new ArrayList<>();
        PageRequest request = PageRequest.of(criteria.getPage(), criteria.getSize());
        List<Statistics> courses = repository.findByStatisticsType(request,type).stream().toList();
