...
public class FAQServiceImpl extends AbstractService<FAQMapper, FAQRepository> implements FAQService {
    ...
    public FAQGetDTO getWithGetDTO(UUID code) {
        FAQ faq = checkExistenceAndGetById(code);
        return mapper.toGetDTO(faq);
    }
    ...
    public List<FAQLocalizedDTO> listByCourseCode(FAQCriteria criteria, String lang, UUID courseCode) {
        PageRequest request = PageRequest.of(criteria.getPage(), criteria.getSize());
        Page<FAQ> faqs = new PageImpl<>(repository.findAllByCourseCode(courseCode), request, repository.count());
        return getLocalizedDTOs(faqs.getContent(), lang);
    }
}
